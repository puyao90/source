...
//创建一个10个字节的Buffer
//var buf2 = new Buffer(10) //Buffer的所有构造函数都是不推荐使用的
...
fs.open("hello.txt2", "w", function (err, fd) {
  ...
  if (!err) {
    ...
    fs.write(fd, "这是异步写入的内容", function (err) {
      //写入的回调函数里不需要fd了，回调会接收到参数 (err, written, string)
      if (!err) {
        console.log("写入成功");
      }
...
var rs = fs.createReadStream(
  "/Users/sphy/English/Password Readers/Password_Readers_5_www.frenglish.ru_Sense_and_Sensibility/01.mp3"
);
