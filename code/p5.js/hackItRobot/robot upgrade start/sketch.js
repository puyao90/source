...
function setup() {
  ...
  var maxIqIndex = 0;
  for (var i = 0; i < robots.length; i++) {
    if (robots[i].iq > maxIq) {
      maxIq = robots[i].iq;
      maxIqIndex = i;
    }
  }
  console.log(
    robots[maxIqIndex].firstName + robots[maxIqIndex].secondName + "iq=" + maxIq
  );
  highlight(robots[maxIqIndex], 0, 0, 255);

  //4. Select the robot with the smallest serial number and highlight it in purple, Also, print it's name to the console
  var smallestSNum = null;
  var smallestSNumIndex;
  for (var i = 0; i < robots.length; i++) {
    if (smallestSNum == null || robots[i].serialNumber < smallestSNum) {
      smallestSNum = robots[i].serialNumber;
      smallestSNumIndex = i;
    }
  }
  console.log(
    robots[smallestSNumIndex].firstName +
      robots[smallestSNumIndex].secondName +
      "smallest serial number = " +
      smallestSNum
  );
  highlight(robots[smallestSNumIndex], 128, 0, 128);
  ...
  for (var i = 0; i < robots.length; i++) {
    for (var j = 0; j < robots[i].abilities.length; j++) {
      if (robots[i].abilities[j] == "destroy humanity") {
        ...
        console.log(
          robots[i].firstName + robots[i].secondName + robots[i].abilities
        );
      }
    }
  }
...
function fillRobots() {
  ...
  robots[round(random(0, robots.length - 1))].abilities.splice(
    random(0, 4),
    1,
    "destroy humanity"
  );
...
function drawRobot(physical) {
  ...
  fill(255, 0, 0);
  triangle(
    x + physical.headWidth * 0.5,
    y + 35,
    x + physical.headWidth * 0.35,
    y + 60,
    x + physical.headWidth * 0.65,
    y + 60
  );
  ...
  vertex(x + physical.headWidth * 0.5, y + 85);
