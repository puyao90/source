...
function draw() {
  ...
  line(0, robotYs[0] - 20, width, robotYs[0] - 20);
  ...
  for (var i = 0; i < robotXs.length; i++) {
    ...
    if (robotXs[i] <= 1150 - 104) {
      var step = random(7);
      if (robotXs[i] + step > 1150 - 104) {
        step = 1150 - 104 - robotXs[i];
      }
      robotXs[i] += step;
    }

    //TODO Check if the robot has won and display message!
    if (robotXs[i] >= 1150 - 104 && someone_won == -1) {
      someone_won = i;
    }
    if (someone_won > -1) {
      // console.log(someone_won);
      textSize(40);
      fill(255);
      strokeWeight(1);
      text(
        "Robot in lane " + (someone_won + 1) + " has won the race",
        100,
        400
      );
    }
