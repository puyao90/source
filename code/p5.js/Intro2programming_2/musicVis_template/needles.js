  ...
  this.draw = function () {
    ...
    for (var i = 0; i < this.plotsDown; i++) {
      for (var j = 0; j < this.plotsAcross; j++) {
        ...
        var w = (width - 3 * this.pad) / 2;
        var h = (height - 3 * this.pad) / 2;

        //draw a rectangle at that location and size
        rect(x, y, w, h);

        //add on the ticks
        this.ticks(x + this.dialRadius, y + h, this.frequencyBins[i * 2 + j]);

        var energy = fourier.getEnergy(this.frequencyBins[i * 2 + j]);

        //add the needle
        this.needle(energy, x + this.dialRadius, y + h);
