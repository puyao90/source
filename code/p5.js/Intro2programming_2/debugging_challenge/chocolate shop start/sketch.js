...
function setup() {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    shopStock.addStock(l.name, l.imageFile, l.weight, l.price, l.quantity, display);
...
function ChocolateBar(name, imageFile, weight, price, display) {
  ...
  this.draw = function () {
    ...
    var pricePer100g = Number.parseFloat((price / weight) * 100).toFixed(2);
    var priceString = '£' + this.price + 'p ( £' + pricePer100g + 'p per 100 grams)';
    textAlign(CENTER);
    fill(0);
