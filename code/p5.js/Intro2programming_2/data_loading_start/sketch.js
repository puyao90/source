...
function Scatter(x, y, width, height) {
  ...
  this.setupAxis = function (xMin, xMax, yMin, yMax, xTickSpacing, yTickSpacing) {
    ...
    //plot the x axis
    line(this.plotX, this.plotY + this.plotHeight, this.plotX + this.plotWidth, this.plotY + this.plotHeight);
    ...
    for (var i = this.plotY; i < this.plotY + this.plotHeight + 1; i += scaledYTickSpacing) {
  ...
  this.lineOfBestFit = function (xValues, yValues, colour) {
    ...
    if (xValues.length == yValues.length) {
      ...
      var m = (xValues.length * sumXY - sumX * sumY) / (xValues.length * sumX2 - sumX * sumX);
      var b = (sumY - m * sumX) / xValues.length;
      console.log('y=' + m + 'x+' + b);

      var scaleB = map(b, this.yMin, this.yMax, this.plotHeight + this.plotY, this.plotY);
