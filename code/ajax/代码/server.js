...
app.get('/server', (request, response) => { //request是对请求报文的封装，response是对响应报文的封装
    //当客户端浏览器向服务器发送请求时，如果请求行的第二段内容，URL路径是/的话，就会执行后面回调函数的代码，由response做出响应
...
app.all('/server', (request, response) => { //post改为all 可以接收任意类型的请求  自定义响应头后会给服务器发一个OPTIONS类型的请求以校验头信息
    response.setHeader('Access-Control-Allow-Origin', '*')
    response.setHeader('Access-Control-Allow-Headers','*')//设置允许自定义响应头
    response.send('HELLO AJAX POST')
})

app.all('/json-server', (request, response) => { //post改为all 可以接收任意类型的请求  自定义响应头后会给服务器发一个OPTIONS类型的请求以校验头信息
    response.setHeader('Access-Control-Allow-Origin', '*')
